...
export const InputCourse = ({ coursesList, courseSelected, setCourseSelected }) => {
    ...
    const handleCourse = (value) => {
        console.log(value)
        console.log('HOLA')
        const course = coursesList.find(course => course.value === value);
        setCourseSelected({...courseSelected, course: { '_id': course.value, 'name': course.description, path: `${course.description}.png` } });
    }
            ...
            {coursesList && (
                <RadioGroup value={courseSelected?._id} onChange={(value) => handleCourse(value)}>
                    ...
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                        {coursesList.map(({ value, description }) => (
                            <RadioGroup.Option
                                key={value}
                                value={value}
                                className={({ active }) =>
